...
const SITE_BASE_URL = "https://nafatech.web.id";
...
// ---------- ANTRIAN ----------
// CATATAN ALUR TERBARU: customer bayar dulu via Midtrans saat order masuk,
// baru NAFA Tech mengerjakan. Jadi antrian kerja di sini SENGAJA hanya
// menampilkan order yang payment_status-nya sudah 'paid' — order yang
// masih 'unpaid'/'pending' (belum/gagal bayar) tidak akan numpuk di sini
// ataupun ke-skip otomatis dikerjakan duluan.
...
// Cari order berikutnya untuk dikerjakan — HANYA yang sudah dibayar
// ('paid'), supaya order yang belum/gagal bayar tidak ikut ke-skip
// otomatis mulai dikerjakan.
async function autoStartNext(){
  ...
  const { data: nextUp } = await sb.from('orders').select('*')
    .eq('status','waiting')
    .eq('payment_status','paid')
...
document.getElementById('addOrderBtn').addEventListener('click', async () => {
  ...
  const { error } = await sb.from('orders').insert({
    ...
    // Order yang diinput manual oleh admin di sini dianggap sudah beres
    // urusan bayarnya di luar sistem (COD/transfer langsung/dll), jadi
    // langsung ditandai 'paid' supaya masuk antrian kerja seperti biasa.
    payment_status: 'paid'
  });
...
// Dengan alur terbaru, ini dipakai sebagai FALLBACK — misalnya kalau link
// pembayaran pertama dari customer sudah kedaluwarsa/gagal, admin bisa
// buatkan ulang link baru dari sini lalu kirim manual ke customer.
